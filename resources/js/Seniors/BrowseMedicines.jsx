...
export default function BrowseMedicines() {
    ...
    const [userRole, setUserRole] = useState("");  // Store the user role (senior, staff, admin)
    ...
    async function load() {
        ...
        try {
            ...
            // Fetch the user's role (this could be passed from your backend or authentication system)
            const userData = await apiFetch("/api/user");
            setUserRole(userData.user.roles[0]?.name);  // Assuming roles are available in the user data
        } catch (e) {
                                ...
                                <tbody>
                                    {filtered.length === 0 ? (
                                        ...
                                        filtered.map((m) => {
                                            ...
                                            else if (totalQuantity < 10) availability = "Low Stock";
