...
export default function SeniorEdit() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (!alive) return;
                setErrors({
                    general: [e?.message || "Failed to load senior"],
                });
            } finally {
    ...
    const submit = async (e) => {
        ...
        try {
            const payload = { ...form, age: computedAge };
    ...
    const handleDelete = async () => {
        ...
        const ok = confirm(
            "Delete this senior? This will remove BOTH the user and the senior profile."
        );
        if (!ok) return;
        ...
        try {
            ...
            setErrors(
                e?.data?.errors || {
                    general: [e?.message || "Delete failed"],
                }
            );
        } finally {
            setDeleting(false);
...
function BloodTypeSelect({ label, value, onChange, items, disabled }) {
    return (
        <div>
            <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 6 }}>
                {label}
