// resources/js/Products/Outstock.jsx
import React, { useEffect, useMemo, useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { apiFetch, normalizeList, safeArray } from "../utils/api";

export default function OutstockProducts() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch("/api/products/outstock");
      const list = normalizeList(res, ["products"]);
            setItems(list);
    } catch (e) {
      setErr(e.message || "Failed to load outstock products");
    } finally {
      setLoading(false);
    }
  };
                  ...
                  <table className="table table-hover table-center mb-0">
                    ...
                    <tbody>
                      {safeArray(filtered).length === 0 ? (
                        <tr><td colSpan={7} className="text-center text-muted">No outstock products</td></tr>
