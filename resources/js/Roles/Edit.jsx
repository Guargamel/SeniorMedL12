...
/**
 * React version of roles/edit.blade.php
 *
 * Expects API endpoints (adjust if needed):
 *  - GET  /api/roles/:id            -> {id,name,permissions:[{name} or string]}
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                setRole(roleData?.name || roleData?.role || "");
                const rolePerms = (roleData?.permissions || []).map((p) => (typeof p === "string" ? p : p.name));
                setSelected(rolePerms);

                setPermissions(Array.isArray(permData) ? permData : (permData?.data || []));
            } catch (e) {
                console.error(e);
